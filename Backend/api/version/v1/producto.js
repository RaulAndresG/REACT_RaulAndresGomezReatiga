...
export const getProductoAllV1 = async (req, res, next) => {
    ...
    res.status(200).json({link: "https://http.cat/images/200.jpg", message:"Productos obtenidos", data: await db.collection("productos").find({}).toArray()})
}
...
export const getProductoV1 = async (req, res, next) => {
    ...
    try {
        res.status(200).json({link: "https://http.cat/images/200.jpg", message:"Productos obtenidos", data: await db.collection("productos").find({_id: new ObjectId(req.params.categoriaId)}).toArray()})
    } catch (error) {
